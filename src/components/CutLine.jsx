...
export default function CutLine(props) {
  ...
  function cutLineStyle(orientation, length, index) {
    let sizing;
    if (length > 480) sizing = 200;
    else sizing = 100;
    const border = 10;
    if (orientation === "vertical") {
      let newLeft = "50%";
      if (index % 3 === 0) newLeft = "50% - " + sizing + "px";
      if (index % 3 === 2) newLeft = "50% + " + sizing + "px";
      return {
    ...
    if (orientation === "orizontal") {
      let newTop;
      if (index < 3) newTop = sizing / 2 - border + "px";
      if (index > 2 && index < 6) newTop = "50% - " + border + "px";
      if (index > 5 && index < 9) newTop = "50% + " + (sizing - border) + "px";
